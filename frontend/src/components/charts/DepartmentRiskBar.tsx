...
export default function DepartmentRiskBar({ data }: Props) {
  ...
  data.forEach((emp) => {
    ...
    if (!agg[dept]) {
      agg[dept] = { name: dept, rendah: 0, sedang: 0, tinggi: 0, total: 0 };
    }
    agg[dept][emp.riskCategory]++;
    agg[dept].total++;
  });
    ...
    <div className="h-[300px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} margin={{ top: 20, right: 20, left: -10, bottom: 0 }}>
          ...
          <XAxis dataKey="name" tick={{ fontSize: 11 }} stroke="#9ca3af" interval={0} />
